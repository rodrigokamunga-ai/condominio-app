    ...
    list.innerHTML =
      visibleReports
        .map((report) => {
          ...
          return ` <article class="report-item admin-report-item" > <div class="admin-report-header" > <div class="report-title"> ${escapeHtml( report.titulo || "Sem título" )} </div> <span class="badge admin-report-status ${statusClass(status)}" > ${escapeHtml(status)} </span> </div> <div class="admin-report-main" > <div class="report-meta"> Protocolo: ${escapeHtml( report.protocolo || "Não informado" )} </div> <div class="report-meta"> Morador: ${escapeHtml( report.nome || "Não informado" )} </div> <div class="report-meta"> Unidade: ${escapeHtml( report.unidade || "Não informada" )} </div> <div class="report-meta"> Categoria: ${escapeHtml( report.categoria || "Não informada" )} </div> <div class="report-meta"> Início: ${formatDate( getOpeningDate(report) )} </div> ${ getResolvedDate(report) ? ` <div class="report-meta"> Fim: ${formatDate( getResolvedDate(report) )} </div> ` : "" } </div> <div class="admin-report-actions" > <button class="action-icon-button detail-button" data-id="${report.id}" type="button" title="Detalhar" aria-label="Detalhar ocorrência"  disabled  > 🔍 </button> <button class="action-icon-button timeline-button" data-id="${report.id}" type="button" title="Linha do tempo" aria-label="Linha do tempo"  disabled  > 🕒 </button> <button class="action-icon-button edit-button" data-id="${report.id}" type="button" title="Editar" aria-label="Editar ocorrência"  disabled  > ✏️ </button> <button class="action-icon-button whatsapp-button" data-id="${report.id}" type="button" title="Enviar pelo WhatsApp" aria-label="Enviar pelo WhatsApp"  disabled > 💬 </button> <button class="action-icon-button email-button" data-id="${report.id}" type="button" title="Enviar por e-mail" aria-label="Enviar por e-mail"  disabled  > ✉️ </button> <button class="action-icon-button admin-danger delete-button" data-id="${report.id}" type="button" title="Excluir" aria-label="Excluir ocorrência"  disabled > 🗑️ </button> </div> </article> `;
        })
        .join("");
  }
